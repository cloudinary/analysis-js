 ...
 */
export function analyzeWatermarkDetection(
  client: CloudinaryAnalysisCore,
  request: components.BaseAnalyzeRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.AnalyzeWatermarkDetectionResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => components.BaseAnalyzeRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/analyze/watermark_detection")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.AnalyzeWatermarkDetectionResponse$inboundSchema),
    M.json(202, operations.AnalyzeWatermarkDetectionResponse$inboundSchema),
